...
def get_call_recommendations(call_id: str, db: Session = Depends(get_db)):
    ...
    if not base_call.embedding:
        raise HTTPException(
            status_code=400, detail="Embedding not available for base call"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error parsing embedding: {str(e)}"
        )
    ...
    return {
        "similar_calls": [
            {"call_id": cid, "similarity": round(score, 4)} for cid, score in top_5
        ],
    ...
    leaderboard = [
        {
            ...
            "avg_sentiment": (
                round(row.avg_sentiment, 3) if row.avg_sentiment is not None else None
            ),
            "avg_talk_ratio": (
                round(row.avg_talk_ratio, 3) if row.avg_talk_ratio is not None else None
            ),
